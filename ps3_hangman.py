...
def getGuessedWord(secretWord, lettersGuessed):
    ...
    for char in secretWord:
        guessedWord += char if char in lettersGuessed else ' _ '
    return guessedWord
...
def getAvailableLetters(lettersGuessed):
    ...
    returns: string, comprised of letters that represents what letters have not
      yet been guessed.
    '''
    return ''.join(chr(x + ord('a')) for x in range(26) if chr(x + ord('a')) not in lettersGuessed)
...
def hangman(secretWord):
    ...
    * After each round, you should also display to the user the
      partially guessed word so far, as well as letters that the user has not yet guessed.
    '''
    guesses = 8
